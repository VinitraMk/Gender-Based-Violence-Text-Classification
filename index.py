...
def train_model_in_azure(azexp, azws, azuserenv, model_name, epoch_index, validation_k, model_args_string, preproc_string = '', save_preds = False, filename = '', features = ''):
    ...
    config = ScriptRunConfig(
        ...
        script='train.py',
        arguments=[input_data, output, model_name, save_preds, epoch_index, validation_k, filename, model_args_string, preproc_string, features],
...
def start_validation(data, args, validation_args, test_ids, test_X):
    ...
    if validation_args['validation_type'] == 'normal_split':
        ...
        train_model_in_azure(azexp, azws, azuserenv, args['model'], -1 , 0, model_args_string, '', True, filename, '')
...
def read_args():
    ...
    train, test_X, test_ids = preprocess_data()
    model = make_model(args)
    model_path = f'{config["processed_io_path"]}/models'
    save_model(model, model_path, args['model'])
    start_validation(train, args, validation_args, test_ids, test_X)
